#!/bin/bash
# ==============================================================================
# Mirror Scorpion v3 - GAMES ACTIVATION (ALL-IN-ONE)
# ==============================================================================
...
log() { echo -e "${C_CYN}[GAMES]${C_END} $*"; }
ok() { echo -e "${C_GREEN}  [✔] $*${C_END}"; }

WORKDIR="$HOME/mirror_scorpion_v3"
log "بدء تفعيل كرت الألعاب..."

mkdir -p "$WORKDIR/lib/features/games"
mkdir -p "$WORKDIR/lib/features/home"
cd "$WORKDIR"

# 1. تحديث pubspec.yaml كاملاً وإضافة flutter_chess_board
...
EOPUB
ok "تم تحديث pubspec.yaml بحزمة الشطرنج"

# 2. إنشاء شاشة الألعاب (GamesScreen)
cat << 'EOG' > lib/features/games/games_screen.dart
...
EOG
ok "تم إنشاء GamesScreen بنجاح"

# 3. إعادة كتابة HomeScreen بالكامل لربط كرت الألعاب
...
EOH
ok "تم ربط كرت الألعاب بـ HomeScreen"

# 4. الرفع إلى GitHub
log "جاري رفع كرت الألعاب إلى GitHub..."
git add .
git commit -m "feat(games): activate games screen and chess engine" || echo "لا تغييرات"
git push origin main || echo "فشل الرفع"

ok "تم تفعيل كرت الألعاب بنجاح يا تامر! 🦂🎮✨"
